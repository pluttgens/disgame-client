...
function DiscordClient(options) {
	...
	function messageHeaders() {
		...
		if (self.internals.token) r.authorization = self.internals.token;
		return r;
	}
	...
	function resolveID(ID, callback) {
		...
		for (var server in self.servers) { //If the ID isn't in the UserID : ChannelID cache, let's try seeing if it belongs to a user.
			if ((self.servers[server].members) && ID in self.servers[server].members) return req('post', "https://discordapp.com/api/users/" + self.id + "/channels", {recipient_id: ID}, function(err, res) {
	...
	function req(method, url) {
		...
		return needle.request(method, url, data, { multipart: (typeof(data) === 'object' && !!data.file), headers: messageHeaders(), json: (typeof(data) === 'object') }, callback);
	}
	...
	self.uploadFile = function(input, callback) { /* After like 15 minutes of fighting with Request, turns out Discord doesn't allow multiple files in one message... despite having an attachments array.*/
		checkRS(function() {
			...
			if (isString) file = fs.readFileSync(input.file);
	...
	self.acceptInvite = function(inviteCode, callback) {
		checkRS(function() {
			...
			var createdServers = [];
			self.on('SERVER_CREATED', function (serverID) {
			...
			req('post', "https://discordapp.com/api/invite/" + inviteCode, function(err, res) {
				try {
					...
					if (!self.servers[res.body.guild.id]) {
						...
						if (joinedServers.indexOf(res.body.guild.id) >= 0) {
							throw ("Already joined server: " + res.body.guild.id);
						}
					}
				} catch (e) {
					return handleErrCB(e, callback);
				}
	...
	self.createChannel = function(input, callback) {
		checkRS(function() {
			...
			req('post', "https://discordapp.com/api/guilds/" + serverID + "/channels", payload, function(err, res) {
				try {
	...
	self.deleteChannel = function(input, callback) {
		checkRS(function() {
			req('delete', "https://discordapp.com/api/channels/" + input.channel, function(err, res) {
